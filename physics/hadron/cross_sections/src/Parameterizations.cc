...
double GetHadronNucleonXscPDG(int particlePDG, double mass, double energyKin, int targetPDG)
{
  ...
  double dumy0 = geant::Log(sMand / s0);
...
double GetHadronNucleonTotalXscNS(int particlePDG, double mass, double energyKin, int targetPDG)
{
  ...
  if (particlePDG == 2112 && pORn) {
    if (pLab >= 373.) {
      ...
      NucleonTotalXsc = A0 + B0 * geant::Log(pE) - 11 + 103 * std::pow(sMand, -0.165); // mb
    } else if (pLab >= 10.) {
      B0 = 7.5;
      A0 = 100. - B0 * geant::Log(3.0e7);
    ...
    } else // pLab < 10 GeV/c
    {
      ...
      {
        if (pLab < 0.4) {
          NucleonTotalXsc = 23 + 50 * (std::pow(geant::Log(0.73 / pLab), 3.5));
        } else if (pLab < 0.73) {
          NucleonTotalXsc = 23 + 50 * (std::pow(geant::Log(0.73 / pLab), 3.5));
        } else if (pLab < 1.05) {
          double lp73     = geant::Log(pLab / 0.73);
      ...
      {
        if (pLab < 0.02) {
          NucleonTotalXsc = 4100 + 30 * std::pow(geant::Log(1.3 / pLab), 3.6); // was as pLab < 0.8
        } else if (pLab < 0.8) {
          NucleonTotalXsc = 33 + 30 * std::pow(geant::Log(pLab / 1.3), 4.0);
        } else if (pLab < 1.05) {
          NucleonTotalXsc = 33 + 30 * std::pow(geant::Log(pLab / 0.95), 2.0);
        } else if (pLab < 1.4) {
          NucleonTotalXsc = 33 + 30 * std::pow(geant::Log(pLab / 0.95), 2.0);
        } else // 1.4 < pLab < 10.  )
        {
  ...
  {
    if (proton) {
      NucleonTotalXsc = 35.45 + B * std::pow(geant::Log(sMand / s0), 2.) + 42.53 * std::pow(sMand, -eta1) +
                        33.34 * std::pow(sMand, -eta2);
    }
    if (neutron) // ???
    {
      NucleonTotalXsc = 35.80 + B * std::pow(geant::Log(sMand / s0), 2.) + 40.15 * std::pow(sMand, -eta1) +
  ...
  } else if (particlePDG == 211 && pORn) // pi+ /////////////////////////////////////////////
  {
    if (proton) // pi+ p
    {
      if (pLab < 0.28) {
        ...
        double lp77     = geant::Log(pLab / 0.77);
      ...
      {
        NucleonTotalXsc = 10.6 + 2. * geant::Log(pE) + 25 * std::pow(pE, -0.43); // ns original
      } else                                                                     //  pLab > 100 // my
      {
        NucleonTotalXsc = GetHadronNucleonXscPDG(particlePDG, mass, energyKin, targetPDG) / geant::units::millibarn;
      }
    ...
    if (neutron) // pi+ n = pi- p??
    {
      ...
      } else if (pLab < 0.395676) // first peak
      {
        ...
        NucleonTotalXsc = 26 + 110 * (geant::Log(pLab / 0.48)) * (geant::Log(pLab / 0.48));
      } else if (pLab < 0.65) {
        NucleonTotalXsc = 26 + 110 * (geant::Log(pLab / 0.48)) * (geant::Log(pLab / 0.48));
      } else if (pLab < 0.72) {
        NucleonTotalXsc = 36.1 + 10 * geant::Exp(-(pLab - 0.72) * (pLab - 0.72) / 0.06 / 0.06) +
      ...
      {
        NucleonTotalXsc = 36.1 + 0.079 - 4.313 * geant::Log(pLab) +
                          3 * geant::Exp(-(pLab - 2.1) * (pLab - 2.1) / 0.4 / 0.4) +
  ...
  } else if (particlePDG == -211 && pORn) /// pi- ////////////////////////////////////////////
  {
    ...
    if (proton) // pi- p
    {
      ...
      } else if (pLab < 0.395676) // first peak
      {
        ...
        double lp48     = geant::Log(pLab / 0.48);
    ...
    NucleonTotalXsc = 35.20 + B * std::pow(geant::Log(sMand / s0), 2.) - 199. * std::pow(sMand, -eta1) +
                      264. * std::pow(sMand, -eta2);
  } else if (particlePDG == 22 && pORn) // modify later on
  {
    NucleonTotalXsc = 0.0 + B * std::pow(geant::Log(sMand / s0), 2.) +
...
double GetHadronNucleonInelasticXscNS(int particlePDG, double mass, double energyKin, int targetPDG)
{
  ...
  if (particlePDG == 2112 && pORn) {
    if (pLab >= 373.) {
      ...
      NucleonElasticXsc = 6.5 + 0.308 * std::pow(geant::Log(sMand / 400.), 1.65) + 9.19 * std::pow(sMand, -0.458);
    } else if (pLab >= 100.) {
      B0 = 7.5;
      A0 = 100. - B0 * geant::Log(3.0e7);

      NucleonTotalXsc = A0 + B0 * geant::Log(pE) - 11
                        // + 103*std::pow(2*0.93827*pE + pM*pM+0.93827*0.93827,-0.165);        //  mb
                        + 103 * std::pow(sMand, -0.165); //  mb

      NucleonElasticXsc = 5.53 + 0.308 * std::pow(geant::Log(sMand / 28.9), 1.1) + 9.19 * std::pow(sMand, -0.458);
    } else if (pLab >= 10.) {
      B0 = 7.5;
      A0 = 100. - B0 * geant::Log(3.0e7);
    ...
    } else // pLab < 10 GeV/c
    {
      ...
      if (proton) // pn to be np
      {
        if (pLab < 0.02) {
          ...
          double l5p        = geant::Log(0.511 / pLab);
          NucleonTotalXsc   = 33 + 30 * std::pow(geant::Log(pLab / 0.95), 2.0);
  ...
  } else if (particlePDG == 211 && pORn) // pi+ /////////////////////////////////////////////
  {
    if (proton) // pi+ p
    {
      if (pLab < 0.28) {
        ...
        NucleonTotalXsc   = 88 * lp77 * lp77 + 14.9;
        NucleonElasticXsc = NucleonTotalXsc * geant::Exp(-3. * (pLab - 0.68));
      } else if (pLab < 1.15) {
        double lp77 = geant::Log(pLab / 0.77);
    ...
    if (proton) // pi- p
    {
      ...
      } else if (pLab < 2.6) // < 3.0) // ns original
      {
        NucleonTotalXsc = 36.1 + 0.079 - 4.313 * geant::Log(pLab) +
                          3 * geant::Exp(-(pLab - 2.1) * (pLab - 2.1) / 0.4 / 0.4) +
                          1.5 * geant::Exp(-(pLab - 1.4) * (pLab - 1.4) / 0.12 / 0.12);
        NucleonElasticXsc = 3. + 13. / pLab; // *geant::Log(pLab*6.79);
      } else                                 // mb
      {
        NucleonTotalXsc   = GetHadronNucleonXscPDG(particlePDG, mass, energyKin, targetPDG) / geant::units::millibarn;
        NucleonElasticXsc = 3. + 13. / pLab;
