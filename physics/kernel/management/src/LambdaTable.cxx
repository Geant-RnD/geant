...
void LambdaTable::GenerateEnergyGrid(const MaterialCuts *matcut, struct LambdaTableForAMaterialCuts *data)
{
  if (!data) {
    ...
    double scl = geant::Log(emax / emin);
    ...
    fNumLambdaTableBins = std::lrint(nbin * geant::Log(fMaxLambdaTableEnergy / fMinLambdaTableEnergy) / scl) + 1;
    // fProcess->GetPhysicsParameters()->GetNumLambdaTableBinsPerDecade()
    //*std::lrint(geant::Log10(fMaxLambdaTableEnergy/fMinLambdaTableEnergy))+1;
    if (fNumLambdaTableBins < 3) {
      fNumLambdaTableBins = 3;
    }
    fEnergyGrid.resize(fNumLambdaTableBins);
    fLogMinLambdaTableEnergy = geant::Log(fMinLambdaTableEnergy);
    double delta             = geant::Log(fMaxLambdaTableEnergy / fMinLambdaTableEnergy) / (fNumLambdaTableBins - 1.0);
    ...
    for (int i = 1; i < fNumLambdaTableBins - 1; ++i) {
      fEnergyGrid[i] = geant::Exp(fLogMinLambdaTableEnergy + i * delta);
    }
  } else {
    // generate the kinetic energy grid common for each material
    double emin = fProcess->GetPhysicsParameters()->GetMinLambdaTableEnergy();
    ...
    data->fNumLambdaTableBins =
        std::lrint(nbin * geant::Log(data->fMaxLambdaTableEnergy / data->fMinLambdaTableEnergy) / scl) + 1;
    //                                fProcess->GetPhysicsParameters()->GetNumLambdaTableBinsPerDecade()
    //                                *std::lrint(geant::Log10(data->fMaxLambdaTableEnergy/data->fMinLambdaTableEnergy))+1;
    if (data->fNumLambdaTableBins < 3) {
      data->fNumLambdaTableBins = 3;
    }
    data->fEnergyGrid.resize(data->fNumLambdaTableBins);
    data->fLogMinLambdaTableEnergy = geant::Log(data->fMinLambdaTableEnergy);
    double delta =
        geant::Log(data->fMaxLambdaTableEnergy / data->fMinLambdaTableEnergy) / (data->fNumLambdaTableBins - 1.0);
    ...
    for (int i = 1; i < data->fNumLambdaTableBins - 1; ++i) {
      data->fEnergyGrid[i] = geant::Exp(data->fLogMinLambdaTableEnergy + i * delta);
    }
