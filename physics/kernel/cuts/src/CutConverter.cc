...
void CutConverter::Initialise()
{
  ...
  double delta = geant::Log(fMaxCutEnergy / fMinCutEnergy) / (fNumEBins - 1);
  double base  = geant::Log(fMinCutEnergy) / delta;
  fEnergyGrid  = new double[fNumEBins]();
  int i        = 0;
  for (; i < fNumEBins - 1; ++i)
    fEnergyGrid[i] = geant::Exp((base + i) * delta);
...
void CutConverter::BuildLengthVector(const Material *mat)
{
  ...
  double deltae = geant::Log(fMaxCutEnergy / fMinCutEnergy);
  deltae /= (fNumEBins - 1);
