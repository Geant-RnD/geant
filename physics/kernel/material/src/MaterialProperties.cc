...
void MaterialProperties::ComputeIonizationParameters()
{
  ...
  if (fMeanExcitationEnergy <= 0.0) {
    ...
    for (int i = 0; i < numElems; ++i) {
      ...
      fMeanExcitationEnergy += fNumOfAtomsPerVolVect[i] * zeff * geant::Log(meanExcEnerZ);
    }
    fMeanExcitationEnergy = geant::Exp(fMeanExcitationEnergy / fTotalNumOfElectronsPerVol);
  }
}
...
void MaterialProperties::ComputeDensityEffectParameters()
{
  ...
  const double twolog10 = 2.0 * geant::Log(10.0);
  ...
  if (indx < 0 && numElems == 1) {
    ...
    if (indx > 0) { // simple NIST material DenistyEffectData was found with the correct state
      ...
      corrFactor        = geant::Log(fMaterial->GetDensity() / orgDensity);
  ...
  if (indx >= 0) {
    ...
    // fDensityEffectParameterC       = 2.0*geant::Log(fMeanExcitationEnergy/fPlasmaEnergy) + 1;
    ...
    fDensityEffectParameterC = 2.0 * geant::Log(fMeanExcitationEnergy / fPlasmaEnergy) + 1.0;
  ...
  if (matState == MaterialState::kStateGas) {
    ...
    corrFactor = geant::Log(curPressure * kSTPTemperature / (curTemperature * kSTPPressure));
...
double MaterialProperties::GetDensityEffectFunctionValue(const double atx)
{
  ...
  if (atx <
    ...
    if (fDensityEffectParameterDelta0 > 0.0) // delta(X) = delta(X_0) 10^{2(X-X_0)} = delta(X_0) exp[2ln(10)(X-X_0)]
      delta = fDensityEffectParameterDelta0 * geant::Exp(twolog10 * (atx - fDensityEffectParameterX0));
  } else if (atx >= fDensityEffectParameterX1) {
    delta = twolog10 * atx - fDensityEffectParameterC;
  } else {
    delta =
        twolog10 * atx +
        fDensityEffectParameterA * geant::Exp(fDensityEffectParameterM * geant::Log(fDensityEffectParameterX1 - atx)) -
...
void MaterialProperties::ComputeRadiationLength()
{
  ...
  const double factorLel   = geant::Log(184.1499);
  const double factorLinel = geant::Log(1193.923);
  ...
  for (int i = 0; i < numElems; ++i) {
    ...
    // compute radlength
    double logZper3 = geant::Log(zet) / 3.0;
    double dum0     = 0.0;
