    ...
    for (int k = 0; k < size; k++) {
      ...
      while (ZK2 < ZLIMI && KPP < (int)x1.size() - 1) {
        ...
        if (E2 - E1 == 0.0 || E2 == 0 || ALPHA == 0.0) {
          std::cout << "Doppler fails between " << E1 << "  " << E2 << "  " << Y2 << "  " << ALPHA << "  " << awri
                    << std::endl;
          continue;
        }
        F0K1 = F0K2;
      ...
      // while loop
      if (XSUM < 0) XSUM = 0;
      //      std::cout << HALF * XSUM / Y2 <<" xsum1 \t"<< xss  <<" FTAIL1 \t"<< 2*x2[0] <<" FTAIL2 \t"<< x2[ jloop ]
      //      <<std::endl;
      FTAIL1 = xss * (FTAIL(sqrt(x1[0] * ALPHA), Y) - FTAIL(ZERO, Y));
      XSUM   = XSUM + FTAIL1;
      FTAIL2 = x2[x1.size() - 1] * (FTAIL(sqrt((x1[x1.size() - 1] + 0.1 * x1[x1.size() - 1]) * ALPHA), Y) -
                                    FTAIL(sqrt(x1[x1.size() - 1] * ALPHA), Y));
      XSUM = XSUM + FTAIL2;
      sigma.push_back(HALF * XSUM / Y2);
      //        std::cout << HALF * XSUM / Y2 <<" xsum2 \t"<< HALF * FTAIL1 / Y2 <<" FTAIL1 \t"<< HALF * FTAIL2 / Y2 <<"
      //        FTAIL2 \t"<< x2[ jloop ] <<std::endl;
      if (jloop > 0 && k < size - 1) {
        mloop = 0;
...
double TNudyEndfDoppler::broadMore(std::vector<double> &x1, std::vector<double> &x2, double ixp)
{
  ...
  XSUM               = XSUM + FTAIL1;
  FTAIL2             = x2[x1.size() - 1] * (FTAILX(sqrt((x1[x1.size() - 1] + 0.1 * x1[x1.size() - 1]) * ALPHA), Y) -
                                FTAILX(sqrt(x1[x1.size() - 1] * ALPHA), Y));
  XSUM = XSUM + FTAIL2;
  return HALF * XSUM / Y2;
}
