      ...
      if (LF == 1) {
        ...
        for (int i = 0; i < np2; i++) {
          ...
          for (int cr = 0; cr < size - 1; cr++) {
            //            std::cout << energyFile5[cr] <<"  "<< energyPdfFile5[cr] << std::endl;
            recursionLinearFile5GenEva(energyFile5[cr], energyFile5[cr + 1], energyPdfFile5[cr], energyPdfFile5[cr + 1],
                                       energy);
          }
          //           for(unsigned int cr = 0 ; cr < energyFile5.size() ; cr ++){
          //             std::cout <<"fill1d "<< energy <<"  "<< energyFile5[cr] <<"  "<< energyPdfFile5[cr] <<
          //             std::endl;
          // 	  }
          fillPdf1d();
        }
        ...
        for (int i = 0; i < np2; i++) {
          ...
          int size = energyFile5.size();
          for (int cr = 0; cr < size - 1; cr++) {
            recursionLinearFile5Watt(energyFile5[cr], energyFile5[cr + 1], energyPdfFile5[cr], energyPdfFile5[cr + 1],
                                     energy);
          }
          fillPdf1d();
        }
...
double TNudyEndfEnergy::GetEnergy5(int ielemId, int mt, double energyK)
{
  ...
  //     std::cout << j <<"  "<< energyPdf5OfMts[ielemId][i][min].size() <<"  "<< energyCdf5OfMts[ielemId][i][min][j]
  //     <<"  "<< rnd1 << std::endl;
  //   }
  ...
  if (plk != 0)
    edes = energyOut5OfMts[ielemId][i][min][k - 1] +
           (sqrt(plk2 + 2 * plk * (rnd1 - energyCdf5OfMts[ielemId][i][min][k - 1])) -
            energyPdf5OfMts[ielemId][i][min][k - 1]) /
               plk;
  double emin = energyOut5OfMts[ielemId][i][min][1] +
                fraction * (energyOut5OfMts[ielemId][i][min + 1][1] - energyOut5OfMts[ielemId][i][min][1]);
  double emax = energyOut5OfMts[ielemId][i][min][size - 1] +
                fraction * (energyOut5OfMts[ielemId][i][min + 1][energyCdf5OfMts[ielemId][i][min + 1].size() - 1] -
                            energyOut5OfMts[ielemId][i][min][size - 1]);
  edes = energyOut5OfMts[ielemId][i][min][1] +
         (edes - emin) * (energyOut5OfMts[ielemId][i][min][size - 1] - energyOut5OfMts[ielemId][i][min][1]) /
             (emax - emin);
