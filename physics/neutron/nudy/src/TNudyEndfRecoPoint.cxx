...
TNudyEndfRecoPoint::TNudyEndfRecoPoint(int ielemId, const char *irENDF) : elemId(ielemId), rENDF(irENDF)
{
  ...
  for (int iMat = 0; iMat < nmats; iMat++) {
    ...
    while ((file = (TNudyEndfFile *)iter.Next())) {
      ...
      case 3: {
        ...
        //	      std::cout<< energyMts[j1] <<"  "<< recoNuPh->GetNuTotal(0, energyMts[j1])/(1+sigcap/sigfis) <<
        // std::endl;
        //    }
        // 	    for(int j = 0; j < 150; j++){
...
double TNudyEndfRecoPoint::GetSigmaPartial(int ielemId, int i, double energyK)
{
  ...
  //       std::cout << eneUni[ielemId][j1] <<"  "<< sigUniOfMt[ielemId][i][j1] <<"  "<< sigUniOfMt[ielemId][i].size()
  //       << std::endl;
  if (minp <= 0) return 0;
