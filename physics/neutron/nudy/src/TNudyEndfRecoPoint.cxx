...
//	      std::cout<< fEnergyMts[j1] <<"  "<< recoNuPh->GetNuTotal(0, fEnergyMts[j1])/(1+sigcap/sigfis) << std::endl;
	//    }
// 	    for(int j = 0; j < 150; j++){
...
//       std::cout << fEneUni[ielemId][j1] <<"  "<< fSigUniOfMt[ielemId][i][j1] <<"  "<< fSigUniOfMt[ielemId][i].size() << std::endl;
  if (minp <= 0) return 0;
