...
namespace NudyPhysics{
  ...
  public:
    TNudyInterface ();
    TNudyInterface( const int projCode, const double projKE, const double temp, const std::string isoName,
  ...
  //--------- GETTERS -------
  inline std::string TNudyInterface::GetIsotopeName () { return fIsoName; }
  inline int TNudyInterface::GetProjectileCode () { return fProjCode; }
  inline int TNudyInterface::GetZ () { return ftZ; }
  inline int TNudyInterface::GetA () { return ftA; }
  inline double TNudyInterface::GetProjectileKE () { return fProjKE; }
  inline double TNudyInterface::GetTemp () { return fTemperature; }
  inline bool TNudyInterface::GetIsFissKey() { return fIsFiss; }
  inline std::string TNudyInterface::GetProjID() { return fProjID; }
  //inline std::vector<double> TNudyInterface::GetXSTable() { return fChannelXSArray; }

  //--------- SETTERS ---------
  inline void TNudyInterface::SetIsotopeName ( const std::string &isoName ) { fIsoName = isoName; }
  inline void TNudyInterface::SetProjectileCode ( const int projCode ) { fProjCode = projCode; }
  inline void TNudyInterface::SetZ ( const int tZValue ) { ftZ = tZValue; }
  inline void TNudyInterface::SetA ( const int tAvalue ) { ftA = tAvalue; }
  inline void TNudyInterface::SetProjectileKE ( const double projKE ) { fProjKE = projKE; }
  inline void TNudyInterface::SetTemp (const double temp ) { fTemperature = temp; }
  inline void TNudyInterface::SetCrossSection ( const double XSvalue ) { fXS = XSvalue; }
  inline void TNudyInterface::SetEndfDataFileName ( std::string fileName ) { fEndfFileN = fileName.c_str(); }
  inline void TNudyInterface::SetEndfSubDataFileName ( std::string fileName ) { fEndfSubDataFileName = fileName.c_str(); }
  inline void TNudyInterface::SetRootFileName ( std::string fileName ) { fRootFileName = fileName.c_str(); }
  inline void TNudyInterface::SetIsFissKey( const bool theKey ) { fIsFiss = theKey; }
  inline void TNudyInterface::SetProjID (const std::string &theID ) { fProjID = theID; }
  //inline void TNudyInterface::AppendXS ( const double xsvalue ) { fChannelFiss.push_back(xsvalue); }
  inline void TNudyInterface::SetProcessType ( const geantphysics::NudyProcessType ptype ) { fProcType = ptype; }
