...
void GSMottCorrection::InitMCDataMaterial(const Material *mat)
{
  ...
  for (int iek = 0; iek < gNumEkin; ++iek) {
    ...
    for (int ielem = 0; ielem < numElems; ++ielem) {
      ...
      perMatPerEkin->fMCScreening += nZZPlus1 * geant::Log(Z23 * mcScrCF);
      ...
      perMatPerEkin->fMCFirstMoment +=
          nZZPlus1 * (geant::Log(1. + 1. / mcScrCF) - 1. / (1. + mcScrCF)) * perElemPerEkin->fMCFirstMoment;
      ...
      if (ielem == numElems - 1) {
        ...
        double dumScr               = geant::Exp(perMatPerEkin->fMCScreening / zs);
