...
{
  static const double invlog106 = 1.0 / (6 * geant::Log(10.));
  fNumEnergyBins                = (int)(binsperdecade * geant::Log(fMaxEnergy / fMinEnergy) * invlog106);
  if (fNumEnergyBins < 3) {
    fNumEnergyBins = 3;
  }
  ++fNumEnergyBins;
  ...
  fLogMinEnergy                   = geant::Log(fMinEnergy);
  double delta                    = geant::Log(fMaxEnergy / fMinEnergy) / (fNumEnergyBins - 1.0);
  fEnergyILDelta                  = 1.0 / delta;
  fEnergyGrid[0]                  = fMinEnergy;
  fEnergyGrid[fNumEnergyBins - 1] = fMaxEnergy;
  for (int i = 1; i < fNumEnergyBins - 1; ++i) {
    fEnergyGrid[i] = geant::Exp(fLogMinEnergy + i * delta);
  }
}
