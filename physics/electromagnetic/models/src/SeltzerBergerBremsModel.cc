...
void SeltzerBergerBremsModel::LoadDCSData()
{
  ...
  fLogLoadDCSMinElecEnergy  = geant::Log(fLoadDCSElectronEnergyGrid[0]);
  fInvLogLoadDCSDeltaEnergy = 1. / (geant::Log(fLoadDCSElectronEnergyGrid[1] / fLoadDCSElectronEnergyGrid[0]));
  ...
  for (size_t i = 0; i < numElements; ++i) {
    ...
    fXsecLimits[zet - 1] = GetDXSECValue(zet, geant::Exp(4. * geant::Log(10. * geant::units::MeV)), 0.97);
    fclose(f);
  }
}
                                                         ...
                                                         double electronekin)
{
  ...
  double logikappacr   = geant::Log(1. / kappacr);
  // find electron energy grid index ie such that E_ie <= ekin <_E_ie+1
  int ie        = 0;
  double eresid = GetEkinIndex(electronekin, ie);
  // we will need the element composition of this material
  ...
  double integral = 0.0;
  for (int i = 0; i < fNGL; ++i) {
    double dumx = 1.0 - geant::Exp(glX[i] * logikappacr) * kappacr;
    // double x = geant::Log(dumx+1.0e-12);
                                                       ...
                                                       double gammaprodcutenergy, double electronekin)
{
  ...
  double integral = 0.0;
  for (int i = 0; i < fNGL; ++i) {
    double dumx = 1.0 - geant::Exp(glX[i] * logikappacr) * kappacr; // ez a felso x -nek az exp(x)-e
    // double x = geant::Log(dumx+1.0e-12); // 1-dumx = kappa
                                                   ...
                                                   double r3)
{
  ...
  return std::sqrt(dum1 * geant::Exp(egamma * geant::Log(dum2)) - densityCor);
}
                                                   ...
                                                   geant::TaskData *td)
{
  ...
  const double minXi = geant::Log(gcut * gcut + densityCor);
  //  double maxXi     = geant::Log(eekin*eekin + densityCor);
  const double delXi = geant::Log(eekin * eekin + densityCor) - minXi;
  double val         = 0.;
  double *rndArray   = td->fDblArray;
  do {
    td->fRndm->uniform_array(2, rndArray);
    egamma             = std::sqrt(std::max(geant::Exp(minXi + rndArray[0] * delXi) - densityCor, 0.));
...
void SeltzerBergerBremsModel::SamplePhotonDirection(double elenergy, double &sinTheta, double &cosTheta, double rndm)
{
  ...
  const double cofA = -signc * geant::Exp(geant::Log(delta) / 3.0);
...
void SeltzerBergerBremsModel::BuildSamplingTableForMaterialCut(const MaterialCuts *matcut, int indxlocal)
{
  ...
  for (int ie = 0; ie < numElecEnergies; ++ie) {
    double eekin = geant::Exp(dataMatCut->fLogEmin + ie * delta); // E_kin_i
    ...
    //    const double xiMax      = geant::Log(eekin*eekin+densityCor); // xi(kappa=1)
    const double xiNorm = geant::Log((eekin * eekin + densityCor) / dum0);
    ...
    for (int ikappa = 0; ikappa < fLoadDCSNumReducedPhotonEnergies; ++ikappa) {
      if (fLoadDCSReducedPhotonEnergyGrid[ikappa] > kappac &&
        ...
        const double r       = geant::Log((kappa * kappa * eekin * eekin + densityCor) / dum0) / xiNorm;
    ...
    for (int id = 0; id < numData; ++id) {
      ...
      const double kappa = std::sqrt(geant::Exp(r * xiNorm) * dum0 - densityCor) / eekin;
      double dcs         = 0.;
      for (int ielem = 0; ielem < numElems; ++ielem) {
    ...
    while (numData < fSTNumSamplingPhotEnergies) {
      ...
      for (int i = 0; i < numData - 1; ++i) {
        ...
        const double kappa = std::sqrt(dum0 * geant::Exp(xx * xiNorm) - densityCor) / eekin;
