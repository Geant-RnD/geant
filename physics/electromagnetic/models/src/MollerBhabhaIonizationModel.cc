                                                         ...
                                                         const double primekin)
{
  ...
  const double twolog10inv = 1.0 / (2.0 * geant::Log(10.0));
  ...
  if (fIsElectron) {
    dedx = geant::Log((tau - tauUpLim) * tauUpLim) + tau / (tau - tauUpLim) +
           (0.5 * tauUpLim * tauUpLim + (2.0 * tau + 1.0) * geant::Log(1.0 - tauUpLim / tau)) / gamma2 - 1.0 - beta2;
  } else {
  ...
  // add the common term
  dedx += geant::Log(2.0 * (tau + 2.0) / meanExcEnergy);
  // get the density effect correction term
  double dumx = geant::Log(betagama2) * twolog10inv;
                                                         ...
                                                         const double r1, const double r2, const double r3)
{
  ...
  const double lPrimEkin = geant::Log(primekin);
  //
...
void MollerBhabhaIonizationModel::BuildSamplingTableForMaterialCut(const MaterialCuts *matcut, int indxlocal)
{
  ...
  int numPrimEnergies = fSTNumPrimaryEnergyPerDecade * std::lrint(geant::Log10(maxPrimEnergy / minPrimEnergy)) + 1;
  numPrimEnergies     = std::max(numPrimEnergies, 3);
  double logEmin      = geant::Log(minPrimEnergy);
  double delta        = geant::Log(maxPrimEnergy / minPrimEnergy) / (numPrimEnergies - 1.0);
  ...
  for (int ipe = 0; ipe < numPrimEnergies; ++ipe) {
    double pekin = geant::Exp(logEmin + ipe * delta);
  ...
  if (maxEkin > pcutenergy) {
    ...
    if (fIsElectron) {                                              // Moller scattering i.e. e- + e- -> e- + e-
      ...
      xsec = (epsmax - epsmin) * (1.0 - parC + 1.0 / (epsmin * epsmax) + 1.0 / ((1.0 - epsmin) * (1.0 - epsmax))) -
             parC * geant::Log((epsmax * (1.0 - epsmin)) / (epsmin * (1.0 - epsmax)));
      xsec /= beta2;
...
double MollerBhabhaIonizationModel::ComputeMollerPDF(const double xi, const double pcutenergy, const double primekin)
{
  ...
  const double dum1 = geant::Log(0.5 / dum0);
  const double a    = geant::Exp(xi * dum1) * dum0; // this is eps =  exp(xi*ln(0.5*T_0/T_cut))*T_cut/T_0
...
double MollerBhabhaIonizationModel::ComputeBhabhaPDF(const double xi, const double pcutenergy, const double primekin)
{
  ...
  const double dum0 = pcutenergy / primekin;
  const double dum1 = geant::Log(1.0 / dum0);
  const double a    = geant::Exp(xi * dum1) * dum0; // this is eps =  = exp(xi*ln(T_0/T_cut))*T_cut/T_0
