...
TestNudy1DetectorConstruction::TestNudy1DetectorConstruction(geant::RunManager *runmgr)
    : geant::UserDetectorConstruction(runmgr)
{
  ...
  fTargetRegionIndx      = -1;
  fTargetX  = 10. * geant::units::cm; // set to thickness by default and the half will be taken in ComputeSetup
...
TestNudy1DetectorConstruction::~TestNudy1DetectorConstruction()
{ /* nothing to do */
}
...
void TestNudy1DetectorConstruction::CreateMaterials()
{
...
void TestNudy1DetectorConstruction::ComputeSetup()
{
  fTargetMaterial = geantphysics::Material::NISTMaterial(fTargetMatName);
...
void TestNudy1DetectorConstruction::CreateGeometry()
{
  ...
  // set target logical colume ID and target region index (TestNudy1 application will require it)
