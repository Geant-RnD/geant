...
TestNudy1::TestNudy1(geant::RunManager *runmgr, TestNudy1DetectorConstruction *det, TestNudy1PrimaryGenerator *gun)
...
void TestNudy1::AttachUserData(geant::TaskData *td)
{
  ...
  TestNudy1ThreadDataEvents *eventData = new TestNudy1ThreadDataEvents(fNumBufferedEvents, fNumPrimaryPerEvent);
...
bool TestNudy1::Initialize()
{
  ...
  if (!fDetector) {
    geant::Error("TestNudy1::Initialize", "Geometry not available!");
    return false;
  }
  ...
  if (!fPrimaryGun) {
    geant::Error("TestNudy1::Initialize", "PrimaryGenertaor not available!");
    return false;
  }
...
void TestNudy1::SteppingActions(geant::Track &track, geant::TaskData *td)
{
  ...
  TestNudy1DataPerPrimary &dataPerPrimary =
    ...
    if (isTransmit && charge != 0.) {
      ...
      if (cost > 0.0) {
        ...
        if (theta >= fHist1Min && theta < fHist1Max) {
          ...
          TestNudy1ThreadDataRun &dataRun = (*fDataHandlerRun)(td);
...
void TestNudy1::FinishEvent(geant::Event *event)
{
  ...
  TestNudy1ThreadDataEvents *data = fRunMgr->GetTDManager()->MergeUserData(event->GetSlot(), *fDataHandlerEvents);
  ...
  TestNudy1DataPerEvent &dataPerEvent = data->GetDataPerEvent(event->GetSlot());
  fMutex.lock();
...
void TestNudy1::FinishRun()
{
  ...
  TestNudy1ThreadDataRun *runData = fRunMgr->GetTDManager()->MergeUserData(-1, *fDataHandlerRun);
  //
