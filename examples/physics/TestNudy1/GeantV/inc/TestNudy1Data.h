...
/**
 * @brief   User defined data structures for the TestNudy1 GeantV application to store/handle scoring data.
 * @author  M Novak
 * @date    July 2017
 *
 ...
 * An additional, global data structure, TestNudy1Data is defined in this application to accumulate data that belongs to
...
// Data structure per-primary particle for TestNudy1
...
public:
  ...
  void Clear();
  TestNudy1DataPerPrimary &operator+=(const TestNudy1DataPerPrimary &other);
...
public:
  ...
  void AddDataPerPrimary(TestNudy1DataPerPrimary &data);
...
public:
  ...
  void Clear();
  TestNudy1DataPerPrimary &GetDataPerPrimary(int primindx) { return fPerPrimaryData[primindx]; }
  TestNudy1DataPerEvent &operator+=(const TestNudy1DataPerEvent &other);

private:
  int fNumPrimaryPerEvent;
  std::vector<TestNudy1DataPerPrimary> fPerPrimaryData; // as many as primary particle in an event
};

// Thread local data structure for TestNudy1 to collecet/handle thread local multiple per-event data structures (as many
...
public:
  ...
  //  }
  bool Merge(int evtslotindx, const TestNudy1ThreadDataEvents &other);

  TestNudy1DataPerEvent &GetDataPerEvent(int evtslotindx) { return fPerEventData[evtslotindx]; }
  const TestNudy1DataPerEvent &GetDataPerEvent(int evtslotindx) const { return fPerEventData[evtslotindx]; }
