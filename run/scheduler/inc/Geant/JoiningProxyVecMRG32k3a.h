   ...
   Join( BaseScalarRNGType* trackPrng[], int numStates )
{
   ...
   constexpr int vecSize= vecRng::VectorSize<decltype(dbVec)>(); // = VectorSize<BackendT>();
   ...
   for( int i= 0; i < vecSize; ++i ) {
      ...
      vecRng::MRG32k3a<BackendT>::SetPartOfState( *trackPrng[i]->GetState(), i);
...
void JoiningProxyVecMRG32k3a<BackendT>::Split()
{
   ...
   for( int i= 0; i < vecSize; ++i ) {
     ...
     //     auto partState = vecRng::MRG32k3a<BackendT>::GetPartOfState(i); // Sets full state
     auto partState = vecRng::MRG32k3a<BackendT>::GetStateAt(i); // Sets full state
     ...
     //      fScalarTrackPrng[i]->CopyState(partState);
      fScalarTrackPrng[i]->SetState(&partState);
   ...
   JoiningProxyVecMRG32k3a<BackendT>::JoiningProxyVecMRG32k3a()
{
   ...
   constexpr int vecSize= vecRng::VectorSize<BackendT>();
   vecRng::MRG32k3a<BackendT>::Initialize(); //  fScalarTrackPrng, BackendT::VectorSize );
   for( int i= 0; i < vecSize; ++i )
      fScalarTrackPrng[i] = nullptr;   
}
...
template <typename BackendT>
   ...
   : JoiningProxyVecMRG32k3a<BackendT>()
{
  vecRng::MRG32k3a<BackendT>::Initialize();
   fFullState= false;
